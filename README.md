...
| `MSG_ACK` | `0x02` | mainTerm → sender | 数据确认 |
...
| `WAITING_RREP` | 已发送 RREQ，等待 RREP（超时 2s） |
| `WAITING_ACK` | 已发送 DATA，等待 ACK 确认（超时 2s） |

**行为**：
- 每 10 秒发起一轮传输
- 路由失效/过期（>10s）或首次启动时发送 RREQ 广播
- 收到 RREP 后沿选中路径发送 DATA（载荷 = sampleCounter + 0xAB）
- 收到 ACK 后**强制清空路由**，下一轮重新探测（确保路径实时准确）
- RREP 超时 → 回 IDLE 重试；ACK 超时 → 回 IDLE 并标记路由无效
...
- **在路径中** → 透明转发（LoRa 重发）
- **不在路径中** → 忽略
- **去重保护**：4 条环形缓存记录已转发帧的 `(srcId, destId, pathId, msgType)`，防止乒乓循环
- ACK 转发 2 份（mainTerm 源头发 3 份，分级冗余）

#### Pending 队列

```
MAX_PENDING = 2    ← 最多同时 2 个待转发帧
```
...
| `StatsTracker` | 全局统计追踪器（帧计数、节点统计、链路 RSSI 历史、通信阶段） |
| `ConnectionManager` | WebSocket 连接池管理 + 广播 |
| `periodic_stats` | 每 3 秒推送全量统计 |
...
- **链路样式**：虚线（RREQ）、实线（RREP/DATA/ACK）
- **RSSI 颜色编码**：🟢 > -45 dBm | 🟡 -45~-59 | 🔴 ≤ -60
- **RSSI 边标签**：直接在拓扑图链路上显示 dBm 数值
- **当前路由高亮**：选中路径的链路发光加粗
- **节点点击**：自动跳转节点详情面板
...
- RREP/ACK 不提取 `lastHopRssi`（mainTerm 发出的帧 `data[7]=0x00`）
...
| **非阻塞中继** | relay 使用 pending 队列 + ID 退避，避免多中继同时转发碰撞 |
| **转发去重** | 4 条环形缓存记录已转发帧的 `(src, dest, pathId, msgType)`，阻断乒乓循环 |
| **ACK 分级冗余** | mainTerm 发 3 份 → relay 发 2 份，根据可靠性需求分级 |
...
| **RSSI 不过期覆盖** | 前端 `setLink()` 仅在收到非 null RSSI 时更新，防止 RREP/ACK 的空值覆盖正确值 |
...
| 拓扑图不显示中继节点 | 中继未转发或超出 20s 超时 | 检查 relay 电源和天线，观察串口 `[ HEAR]` 输出 |
