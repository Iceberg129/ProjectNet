...
| `periodic_stats` | 每 3 秒推送全量统计 + **全阶段超时检测**（RREQ 6s / RREP 5s / DATA 5s / ACK 4s） |
...
- **tooltip 悬停**：鼠标移至节点显示设备详情，移至链路显示 RSSI 值
- **节点点击**：自动跳转节点详情面板
...
- **节点垃圾回收**：12s 无活动 → 半透明 + [离线] 标签并清除链路；25s 无活动 → 彻底删除；重新活跃 → 即时恢复
...
| 节点变半透明离线 | 中继掉电/超出 12s 无数据 | 节点恢复数据后自动转为在线 |
