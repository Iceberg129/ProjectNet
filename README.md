...
| `MSG_JOIN` | `0x20` | 新节点 → mainTerm | 入网请求 |
| `MSG_JOIN_OK` | `0x21` | mainTerm → 新节点 | 入网允许（白名单通过） |
| `MSG_JOIN_NO` | `0x22` | mainTerm → 新节点 | 入网拒绝 |
| `MSG_HB` | `0x03` | 各节点 → mainTerm | 心跳保活 |
| `MSG_CMD` | `0x04` | mainTerm → 目标节点 | 下行命令（信道配置） |
| `MSG_CMD_ACK` | `0x05` | 目标节点 → mainTerm | 命令应答 |
| `MSG_KICK` | `0x08` | mainTerm → 目标节点 | 踢出命令（静默节点） |
| `MSG_UNKICK` | `0x09` | mainTerm → 目标节点 | 恢复命令（解除踢出） |
| `MSG_PREPARE` | `0x0A` | mainTerm → 广播 | 两阶段提交：信道准备（全网协调） |
| `MSG_READY` | `0x0B` | 各节点 → mainTerm | 两阶段提交：节点就绪 |
| `MSG_COMMIT` | `0x0C` | mainTerm → 广播 | 两阶段提交：执行切换 |
| `MSG_NODE_STATUS` | `0x07` | 各节点 → mainTerm | 节点运行时状态上报 |

---

## 白名单入网协议

节点上电后必须先完成与主节点的三次握手入网，才能参与 AODV-Lite 通信。

**入网流程：**

1. **JOIN 请求**：新节点（sender/relay）上电后广播 JOIN 帧（`msgType=0x20`），`data[0]` 携带角色码（1=中继，2=终端）。sender 进入 `JOINING` 状态等待应答。
2. **白名单校验**：主节点收到 JOIN 后，从 EEPROM 白名单中查找发送者 ID。
   - **通过** → 回复 JOIN\_OK（`msgType=0x21`，`data[0]=0xAC`）
   - **拒绝** → 回复 JOIN\_NO（`msgType=0x22`，`data[0]` 写入拒绝码：0x01=角色不匹配，0x02=不在白名单）
3. **入网确认**：新节点收到 JOIN\_OK 后置 `joinedNetwork=true`，转入正常通信状态。若超时（3s）或收到 JOIN\_NO，重试最多 3 次。超过最大重试次数则放弃入网直接进入 IDLE（演示兼容模式）。
4. **中继转发**：JOIN 帧由 relay 透明转发（去重但不盖 RSSI 章），确保远端节点也能入网。

**白名单管理（串口命令）：**

白名单存储在 mainTerm 的 EEPROM 中（ATmega328P 内置 1024 字节），首次上电自动写入默认条目（0x21、0x22、0x30）。每条记录 3 字节：`{id, role, kicked_flag}`，其中 `kicked_flag` 在 mainTerm 重启后保持持久化。可通过串口命令在线管理：

| 命令 | 示例 | 说明 |
|------|------|------|
| `WL+<id>:<role>` | `WL+0x30:2` | 添加/更新节点，role=1 中继 / 2 终端 |
| `WL-<id>` | `WL-0x30` | 删除节点 |
| `WL?` | `WL?` | 列出所有白名单条目（WL:N 协议：先发条目总数，再逐行输出，以 `WL:END` 结尾） |
| `KICK:<id>` | `KICK:0x21` | 踢出节点（EEPROM 持久化 + LoRa 发送 KICK 帧 + 前端拓扑移除） |
| `UNKICK:<id>` | `UNKICK:0x21` | 恢复被踢节点（清除 kicked 标志 + LoRa 发送 UNKICK 帧） |

**WL:N 白名单协议：** 主节点 `WL?` 输出格式：
```
WL:N:3
WL:0x21:1:0
WL:0x22:1:1
...
WL:END
```
每条 `WL:hex:role:kicked`，Python 后端解析为结构化 `wl_entry` 事件，前端按预期计数校验完整性，不完整自动重试。

## 信道配置（CMD）

系统支持在运行中远程修改任意节点的 LoRa 参数（频率、扩频因子、发射功率），无需重启或中断通信。

**下发路径：** Browser → WebSocket → Python → Serial → mainTerm → LoRa → 目标节点

**CMD 帧编码（data[8] 紧凑二进制）：**

| 偏移 | 字段 | 说明 |
|------|------|------|
| 0 | relayCount | 中继路径长度（0=直达） |
| 1--2 | relays[2] | 中继 ID 列表 |
| 3--4 | freq | 频率 ×10（uint16 BE），如 5300 = 530.0 MHz |
| 5 | sf | 扩频因子（7--12） |
| 6 | power | 发射功率（dBm, 2--20） |
| 7 | reserved | 保留 |

**CMD_ACK 帧编码：** data[0] = 状态码（0=成功, 1=无效频率, 2=无效SF, 3=无效功率, 4=LoRa初始化失败）

**确认机制：** mainTerm 发送 CMD 后启动 2s 超时等待 CMD_ACK，超时未收到则通过串口输出告警。中继沿路径转发 CMD 和 CMD_ACK。

**串口命令格式：** `CFG:<dest>:F<freq>:S<sf>:P<power>`，例如 `CFG:0x30:F530:S7:P17`

**目标节点处理：** 收到 CMD 帧 → 验证参数范围 → `LoRa.begin(freq)` + `setSpreadingFactor(sf)` + `setTxPower(power)` → 回复 CMD_ACK

## 全网信道切换（两阶段提交）

系统支持通过 Web UI 对整个网络执行协调信道切换（频率 + 扩频因子），采用两阶段提交（2PC）协议保证全网一致性，切换期间 AODV 通信自动暂停。

**切换流程：**

```
Browser                Python                 mainTerm               relay/sender
   │──CFG_ALL:F500:S7──>│───Serial─────────>│                        │
   │                     │                    │──PREPARE(0x0A) 广播─>│
   │                     │                    │  freq=500, sf=7       │──验证参数
   │                     │                    │  每1s重试，最多3次     │──暂存 pendingFreq/Sf
   │                     │                    │                        │──READY(0x0B)──>│
   │                     │                    │<──收集 READY──────────│  (非阻塞退避)
   │                     │                    │  等待 3s 收集窗口     │
   │                     │                    │──COMMIT(0x0C) 广播──>│
   │                     │                    │  发送2次(间隔200ms)   │──LoRa.begin(新参数)
   │                     │                    │  LoRa.begin(新参数)    │──恢复 AODV 通信
   │                     │                    │  清空 nodeOnline[]    │
   │                     │                    │  等待心跳验证(15s)    │
   │                     │                    │──COMMIT result──>     │
   │                     │<──channel_switch───│  逐节点在线状态       │
   │<──result event──────│  _result event     │                        │
```

**READY 非阻塞退避：**
- sender（0x30）：退避 30ms 后发送 READY
- relay（0x21/0x22）：退避 `300 + (ID末4位) × 60 ms`（约 360ms/420ms），错开避免 LoRa 捕获效应
- 各节点收到其他节点的 READY 后自动转发，确保主节点收齐

**PREPARE/COMMIT 重复去重：** Python 后端使用布尔标志 `_two_phase_prepare_sent` / `_two_phase_commit_sent` 去重，前端使用 `_readyNodesSeen` Set 去重 READY。

**切换验证：** COMMIT 后清空 `nodeOnline[]` + 重置 `nodeLastSeen[]`，等待 15s 心跳周期，收集在线状态后通过 `channel_switch_result` 事件反馈。前端显示颜色编码结果横幅（✅ 成功 / ⚠️ 部分成功 / ❌ 失败）。

## 节点运行时状态上报

各节点每 15 秒自动上报运行状态，通过 LoRa → mainTerm → 串口 → Python → WebSocket → 浏览器前端展示。

**状态帧格式（MSG_NODE_STATUS = 0x07, 8 字节数据）：**

| 字节 | 字段 | 说明 |
|------|------|------|
| 0-1 | uptime | 运行时长（秒，uint16 BE） |
| 2-3 | freeRAM | 空闲 SRAM（字节，uint16 BE） |
| 4-5 | txPkts | LoRa 发送包累计计数 |
| 6-7 | rxPkts | LoRa 接收包累计计数 |

**上报路径：**
- relayTerm/sender → LoRa STATUS 帧 → mainTerm `Serial.write()` 转发 → Python 解析二进制帧
- mainTerm 自身 → 串口文本行 `NODE_STATUS:uptime:freeRAM:txPkts:rxPkts:freq:sf`

**前端展示：** 点击拓扑图中节点 → 「🔍 节点」面板显示：运行时长（h/m/s）、空闲内存（颜色编码）、收发包计数、当前信道参数（仅主节点）。状态每 15s 自动更新。

## 心跳保活（HB）

各入网节点每 10s ± 2s 随机抖动向 mainTerm 发送 HB (0x03) 心跳帧，mainTerm 跟踪各节点最后活跃时间。

| 参数 | 值 | 说明 |
|------|-----|------|
| 发送间隔 | 10s ± 2s 随机 | 多节点天然错开，避免碰撞 |
| 离线判定 | 连续 24s 无心跳 | 容忍单次丢帧 |
| 超时动作 | 打印告警 + Serial 通知 PC → UI 日志 + 拓扑移除 | 前端同步感知 |

**设计理由：** 前端 UI 的 20s 拓扑超时是纯视觉行为，HB 机制补充了固件层的真实节点活跃性感知，使 A(3) 拓扑维护完整闭环。

## 节点踢出/恢复（Kick/Unkick）

系统支持通过 Web UI 或串口命令远程踢出/恢复任意白名单节点。踢出后节点进入静默状态，不参与组网、不被心跳扫描追踪、不出现在拓扑图中。

### 踢出链路

```
Browser                Python                mainTerm              目标节点
   │──KICK:0x21─────────>│───Serial──────>│                        │
   │                     │                 │──wlSetKicked(0x21,1)  │ EEPROM 持久化
   │                     │                 │──nodeOnline[0x21]=0   │ 防心跳离线扫描
   │                     │                 │──LoRa KICK(0x08)─────>│
   │                     │                 │                        │──gKicked=true
   │                     │                 │                        │  进入静默状态
   │                     │                 │──Serial: KICKED 0x21──>│
   ...
   │<──kicked event──────│                 │                        │
   │──stats.remove───────│  清理链路追踪    │                        │
   │  _node_links(0x21)  │  防 link_broken │                        │
```
...
| 层级 | 位置 | 措施 |
|------|------|------|
| ① JOIN 阻断 | mainTerm `handleJOIN()` | 被踢节点请求入网 → 回复 `JOIN_REJ` (0x03) + 重发 KICK |
| ② 帧丢弃 | mainTerm `handlePacket()` | 所有非 JOIN 帧从 `wlIsKicked(srcId)` 节点直接丢弃 |
| ③ 白名单过滤 | mainTerm `handlePacket()` | `wlFind(srcId) < 0` 的非白名单帧丢弃 |
| ④ RREQ 选路跳过 | mainTerm `handleRREQ()` | 跳过印章中含踢出中继 ID 的候选路径 |
| ⑤ 心跳扫描跳过 | mainTerm `loop()` 离线检测 | `wlIsKicked(id) → continue`，不生成离线告警帧 |
| ⑥ 中继 ID 清除 | mainTerm `Serial.write` 前 | RREQ 印章、RREP/DATA/ACK 中继列表中踢出 ID 清零 |
| ⑦ 链路追踪清理 | Python `StatsTracker.remove_node_links()` | 踢出时从 `_known_links`、`_link_fail_count`、`link_rssi`、`rssi_history` 清除所有涉及链路 |

### 踢出节点行为

- **relayTerm**：`gKicked=true` → 静默：不转发、不发心跳，但仍监听 LoRa 等待 UNKICK 恢复帧
- **sender**：`discState=KICKED` → 静默：不收发包（同样监听 UNKICK）
- **mainTerm**：EEPROM 持久化 `kicked_flag`，重启后踢出状态不丢失
- **前端**：收到 `kicked` 事件 → 从拓扑中移除节点及链路、清除路由高亮、更新白名单面板
- **链路事件**：`link_broken` / `link_retry` / `node_offline` 均跳过已踢出节点（后端清理 + 前端防御双重保障）

### 恢复

UNKICK 命令清除 `kicked_flag` → 发送 `MSG_UNKICK` 帧 → 节点恢复 IDLE → 可重新参与组网。EEPROM 持久化：恢复状态在 mainTerm 重启后保留。
...
| `WL?` | 列出 EEPROM 白名单所有条目（WL:N 协议） |
| `WL+<id>:<role>` | 添加/更新白名单节点（如 `WL+0x30:2`，role=1中继/2终端） |
| `WL-<id>` | 删除白名单节点（如 `WL-0x30`） |
| `KICK:<id>` | 踢出节点（如 `KICK:0x21`），简写 `K:0x21` |
| `UNKICK:<id>` | 恢复被踢节点（如 `UNKICK:0x21`），简写 `UK:0x21` |
| `CFG:<dest>:F<freq>:S<sf>:P<power>` | 信道配置命令（如 `CFG:0x30:F530:S7:P17`），向目标节点下发 LoRa 参数 |
| `CFG_ALL:F<freq>:S<sf>` | 全网两阶段提交信道切换（如 `CFG_ALL:F500:S7`），主节点广播 PREPARE→收集 READY→COMMIT 执行 |
...
| `text_log` | server→client | 串口文本行（仅转发含关键字的行：TOPO/STATS/ROUTE/RSSI 等，过滤噪音，含 sender 重试信息） |
| `wl_entry` | server→client | **白名单单个条目**（WL:N 协议解析后逐条推送：nodeId/role/kicked） |
| `wl_end` | server→client | **白名单读取完成**（含 expectedCount，前端校验完整性，不完整自动重试） |
| `kicked` | server→client | **节点被踢出**（mainTerm 确认后推送，前端移除拓扑 + 更新白名单面板） |
| `unkicked` | server→client | **节点被恢复**（mainTerm 确认后推送，前端更新白名单面板） |
...
| `two_phase_progress` | server→client | **两阶段提交进度**（prepare/ready/commit 各阶段实时推送） |
| `channel_switch_result` | server→client | **信道切换结果**（逐节点就绪/在线状态 + 成功率判定） |
| `node_status` | server→client | **节点运行时状态**（运行时长/空闲内存/收发包计数/信道参数） |
| `cmd_ack` | server→client | **CMD 命令应答**（目标节点配置结果反馈） |
...
```
┌──────────────────────────────────────────────────────────┐
│  HEADER: 标题 + 串口选择 + 波特率 + 信道/白名单/导出按钮    │
├──────────────────────────────────────────────────────────┤
...
- **节点运行时状态**：每 15s 自动上报各节点的运行时长、空闲内存、收发包计数等信息，实时展示在节点详情面板
- **信道配置面板**：功率单节点即时调整 + 频率/SF 全网两阶段协调切换，含进度 UI 和结果横幅
...
| **踢出七层隔离** | JOIN 阻断 → 帧丢弃 → 白名单过滤 → RREQ 选路跳过 → 心跳扫描跳过 → 中继 ID 清除 → 链路追踪清理，层层拦截确保被踢节点彻底静默 |
| **踢出 EEPROM 持久化** | 白名单条目第三字节存储 `kicked_flag`，mainTerm 重启后踢出状态不丢失，被踢节点重新入网被自动拒绝 |
| **WL:N 计数协议** | `WL?` 先发条目总数 `WL:N:n`，再逐行输出，Python 解析为结构化事件，前端按 expectedCount 校验完整性，不完整自动重试 |
| **String → char[]** | 串口命令解析用固定 `char[32]` 缓冲区替代 Arduino `String` 类，避免堆碎片挤占 ATmega328P 仅 322B 栈余量 |
| **两阶段提交信道切换** | PREPARE（每1s重试×3）→ 收集 READY（非阻塞退避）→ COMMIT（连发2次间隔200ms）→ 验证心跳15s，保证全网协调切换时无节点掉队 |
| **READY 非阻塞退避** | sender 30ms / relay 360-420ms 退避发送 READY，避免 LoRa 捕获效应导致多节点碰撞；收到其他节点 READY 自动转发 |
| **节点状态定期上报** | 每 15s 各节点上报运行时状态（运行时长/内存/收发包），全链路 LoRa → Serial → Python → WebSocket → 前端，查看节点时实时刷新 |
...
| 踢出后仍显示"链路断开" | 后端 `_known_links` 保留踢出节点链路，周期末判定缺失 | 已修复：`remove_node_links()` 踢出时清理 + 前端跳过 `_wlKicked` 节点 |
| 白名单读取不完整 | 串口 TX 缓冲区溢出丢行 | 已修复：WL:N 计数协议 + `delay(2)` + `Serial.flush()` + 前端 expectedCount 校验自动重试 |
| 踢出节点心跳帧触发入网 | 心跳离线扫描绕过 `handlePacket()` 踢出检查 | 已修复：`loop()` 扫描增加 `wlIsKicked(id) continue` + `sendKick()` 设 `nodeOnline[id]=false` |
| 踢出节点 KICK 帧显示"未知" | MSG_KICK 不在命名表中 | 已修复：main.py + index.html 均添加 MSG_KICK/MSG_UNKICK 命名映射 |
...
├── UI/
│   └── UI/
│       ├── main.py              ← FastAPI 后端（串口解析 + WebSocket + 两阶段提交 + 节点状态）
│       └── index.html           ← Web 前端（ECharts 拓扑 + RSSI 趋势 + 日志/节点/白名单/信道面板）
└── Docs/
    ├── design_proposal.tex      ← 组网设计方案（LaTeX 源码）
    ├── design_proposal.pdf      ← 组网设计方案（PDF 排版）
    └── fig_*.jpg                ← 方案配图（架构/帧结构/状态机/通信时序/界面等 12 张）
```
...
### 2026-07-02 — UI 全面交互增强 + 诊断报告系统

**消息流网格（msgGrid）**
- 完全自适应：格子大小 10px，列数/行数根据卡片可用空间动态计算
- 悬停弹窗：180ms 防抖显示帧详情（消息类型、时间、收发方、十六进制码）
- 点击固定高亮：`_pinnedSeq` 数据驱动追踪，随网格平移自动跟随，移出视野自动取消
- 跳转日志：弹窗内「📍 跳转日志」按钮 → 右侧日志面板平滑滚动 + 对应帧颜色高亮 2s
- 消息类型颜色重构：17 种类型全部唯一色，按核心通信/两阶段提交/管理指令/入网流程四组区分

**走势图十字准线交互**
- 悬停隐藏鼠标 + 竖线 + 交点圆点（单线 1 点，速率 2 点）
- 弹窗三层结构：标题（卡片名）→ 介绍（参数含义）→ 数值 + 单位 + 时间
- 在线节点新增走势图（`onlineCount`），30s 跨度右对齐
- SVG 元素只初始化一次（`_sparkReady`），后续仅 `setAttribute` 更新折线，消除闪烁

**串口关闭界面冻结**
- `_uiFrozen` + `_manualClose` 双重标志：关闭串口后卡片/拓扑/阶段全部冻结，日志继续写入
- 打开串口后恢复，页面刷新时自动同步后端串口状态

**信道切换增强**
- 四阶段步骤重命名：PREPARE → READY → COMMIT → VERIFY
- Toast 通知（右上角弹出 3.5s，不依赖面板打开）
- 15s 超时检测 + 按钮自动恢复
- `startChannelSwitch` 直接记录目标信道，不依赖可能被过滤的 PREPARE 事件
- 信道参数 header 居中显示

**在线节点修复**
- 总数从白名单 `_wlEntries.length + 1` 动态获取
- 在线数直接统计 `activeNodes` 中非 stale 节点，与拓扑图一致
- WebSocket 连接 + 首帧到达后自动加载白名单

**诊断报告增强**
- 导出日志新增完整网络快照：信道/串口状态、统计数值、拓扑节点在线状态、链路 RSSI 详情、白名单、节点运行时状态
- 自动保存至服务器 `log/` 文件夹（浏览器下载 + 后端 `POST /api/save_log`）
- 后端 `channel_switch_result` 的 total 从 `readyMask` 位掩码动态计算

**其他**
- 重置按钮：二次确认 → 可选保留/清除日志 → 关闭串口 → 恢复初始状态
- 右上角 WebSocket 状态「在线/离线」→「后端」（只变色不换字）
- 卡片标题 `10px→11px bold`，#94a3b8
- 「速率」→「主节点速率」，介绍「串口数据吞吐量」→「主节点上下行通信速率」
- 弹窗含单位（B/s、dBm、%）
- 节点详情移除频率/SF（属全网参数，非节点属性）
- 速率数字无守卫渲染，与走势图同步

### 2026-07-01 — 移除模拟 + 节点运行时状态上报系统

**移除模拟功能**
- 移除 index.html：模拟按钮、自动演示按钮、`triggerSimulate()` / `stopSimulate()` / `toggleAutoDemo()` 函数
- 移除 main.py：`/api/simulate`、`/api/stop_simulate` 端点及 `simulate_traffic()` 函数
- `clearFrontendState()` / `resetAll()` 清理模拟相关代码

**节点状态上报（MSG_NODE_STATUS = 0x07）**
- 帧格式：data[0-1]=uptime(uint16), data[2-3]=freeRAM(uint16), data[4-5]=txPkts(uint16), data[6-7]=rxPkts(uint16)
- mainTerm：串口文本行 `NODE_STATUS:uptime:freeRAM:txPkts:rxPkts:freq:sf` 每 15s 上报
- relayTerm/sender：LoRa STATUS 帧 → mainTerm Serial 转发 → Python 解析，每 15s 上报
- 三端均新增 `freeMemory()` 函数、`txPacketCount`/`rxPacketCount` 自动追踪
- Python：解析 STATUS 帧 + NODE_STATUS 文本行，缓存 `_node_status_cache`，广播 `node_status` 事件
- 前端：节点详情面板新增「📊 节点运行时状态」区块（运行时长/空闲内存/收发包/信道参数）

### 2026-06-30 #2 — 节点踢出/恢复系统 + 白名单 UI 面板

**固件（mainTerm.ino）**
- 新增 MSG_KICK(0x08) / MSG_UNKICK(0x09) 消息类型：`sendKick()` / `sendUnkick()` 通过 LoRa + 串口双通道发送
- EEPROM 白名单增加第三字节 `kicked_flag`：`wlSetKicked()` / `wlIsKicked()` 持久化踢出状态
- 踢出七层隔离：JOIN 拒绝(0x03) + 帧丢弃 + 白名单过滤 + RREQ 选路跳过 + 心跳离线扫描跳过 + Serial.write 前清除踢出中继 ID 印章/列表 + `nodeOnline[id]=false`
- WL:N 计数协议：`wlPrint()` 先输出 `WL:N:<count>`，每行 `WL:hex:role:kicked`，`delay(2)` + `Serial.flush()` 防丢行
- 串口命令 `KICK:<id>` / `UNKICK:<id>`（及简写 `K:<id>` / `UK:<id>`）
- 串口命令解析 String API → 固定 `char[32]` 缓冲区，避免堆碎片

**固件（relayTerm.ino / sender.ino）**
- 新增 KICKED 状态处理：`gKicked=true` / `discState=KICKED` → 静默（不转发、不发心跳），但仍监听 LoRa 等待 UNKICK 恢复
- 收到 UNKICK 帧后自动恢复 IDLE，可重新参与组网

**后端（main.py）**
- 新增 MSG_KICK / MSG_UNKICK 命名映射（MSG_NAMES / MSG_ZH）
- WL:N 计数前缀解析：`WL:N:n` → `_wl_expected` → `wl_end` 事件含 `expectedCount`
- WL 逐行解析为结构化 `wl_entry` 事件（`nodeId / role / kicked`）
- `KICKED` / `UNKICKED` 串口文本解析 → 广播 `kicked` / `unkicked` 事件
- 新增 `StatsTracker.remove_node_links()`：踢出时清理 `_known_links`、`_link_fail_count`、`link_rssi`、`rssi_history`、`_current_rreq_links`，防止冗余 `link_broken` 日志
- 串口未打开时发送命令返回错误提示

**前端（index.html）**
- 新增白名单管理面板（右侧滑入）：分类展示正常节点（绿色 ✅）+ 已踢出节点（红色 🚫）
- 踢出按钮（`KICK:<id>` → 2s 后自动刷新）、恢复按钮（`UNKICK:<id>` → 2s 后自动刷新）
- 添加节点（`WL+<id>:<role>` → 1.5s 后自动刷新），状态提示栏（颜色编码，5s 自动消失）
- 防抖 wlRefresh（300ms）+ expectedCount 校验 + 不完整自动重试（1.5s）
- `link_broken` / `link_retry` / `node_offline` 事件跳过 `_wlKicked` 中节点（双重防御）
- 新增 "🗑️ 清除" 按钮：一键清空前端所有状态 + 后端统计重置
- 串口关闭时自动清空所有节点/链路状态
- 节点详情面板显示链路数据年龄 + broken 状态

### 2026-06-30 #1 — 智能组网重试与逐链路离线检测
